...
func TestStreamingOverlapAdd(t *testing.T) {
	// Simple kernel
	...
	out1, err := soa.ProcessBlock(block1)
	if err != nil {
		t.Fatalf("ProcessBlock block1 failed: %v", err)
	}

	out2, err := soa.ProcessBlock(block2)
	if err != nil {
		t.Fatalf("ProcessBlock block2 failed: %v", err)
	}

	// Verify continuity - the tail from block1 should appear in block2
	// Block1: convolve [1,0,0,0] with [1,0.5,0.25] = [1, 0.5, 0.25, 0, 0, 0]
	// Output block1: [1, 0.5, 0.25, 0]
	// Tail: [0, 0]
	// Block2: convolve [0,0,0,0] with [1,0.5,0.25] = [0,0,0,0,0,0]
	// Add tail [0,0]: [0,0,0,0,0,0]
	// Output block2: [0, 0, 0, 0]
...
func TestStreamingOverlapAddVsBatch(t *testing.T) {
	...
	// Batch processing
	batchOLA, err := NewOverlapAdd(kernel, blockSize)
	if err != nil {
	...
	streamOLA, err := NewStreamingOverlapAdd(kernel, blockSize)
	if err != nil {
		t.Fatalf("NewStreamingOverlapAdd failed: %v", err)
	}
...
func TestStreamingOverlapAddReset(t *testing.T) {
	...
	out1, _ := soa.ProcessBlock(block1)
	soa.Reset()
	out2, _ := soa.ProcessBlock(block1)
...
func TestStreamingOverlapAddProcessBlockTo(t *testing.T) {
	kernel := []float64{1.0, 0.5, 0.25}
	blockSize := 8
...
func BenchmarkStreamingOverlapAdd(b *testing.B) {
	...
	soa, err := NewStreamingOverlapAdd(kernel, blockSize)
	if err != nil {
		b.Fatalf("NewStreamingOverlapAdd failed: %v", err)
	}
...
func BenchmarkStreamingOverlapAddTo(b *testing.B) {
	kernel := make([]float64, 4096) // Typical IR length
	for i := range kernel {
		kernel[i] = 1.0 / float64(len(kernel))
	}
...
func TestStreamingOverlapAddErrors(t *testing.T) {
	...
	t.Run("ZeroBlockSize", func(t *testing.T) {
		_, err := NewStreamingOverlapAdd([]float64{1.0}, 0)
		if err == nil {
	...
	t.Run("NegativeBlockSize", func(t *testing.T) {
		_, err := NewStreamingOverlapAdd([]float64{1.0}, -1)
		if err == nil {
	...
	t.Run("WrongInputSize", func(t *testing.T) {
		soa, err := NewStreamingOverlapAdd([]float64{1.0, 0.5}, 4)
		if err != nil {
			t.Fatalf("NewStreamingOverlapAdd failed: %v", err)
		}
...
func TestStreamingOverlapAddGetters(t *testing.T) {
	...
	if soa.BlockSize() != blockSize {
		t.Errorf("BlockSize() = %d, want %d", soa.BlockSize(), blockSize)
	}

	if soa.KernelLen() != len(kernel) {
		t.Errorf("KernelLen() = %d, want %d", soa.KernelLen(), len(kernel))
	}

	// FFT size should be next power of 2 >= blockSize + kernelLen - 1
	expectedFFTSize := nextPowerOf2(blockSize + len(kernel) - 1)
	if soa.FFTSize() != expectedFFTSize {
		t.Errorf("FFTSize() = %d, want %d", soa.FFTSize(), expectedFFTSize)
	}
}
...
func TestStreamingOverlapAddLongKernel(t *testing.T) {
	...
	for i := range 3 {
		...
		// Should still have non-zero output from long tail (using higher threshold)
		maxVal := 0.0
		for _, v := range out {
...
func TestStreamingOverlapAddContinuity(t *testing.T) {
	...
	batchOLA, _ := NewOverlapAdd(kernel, blockSize)
