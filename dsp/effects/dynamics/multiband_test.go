...
func TestNewMultibandCompressorWithConfig(t *testing.T) {
	t.Run("valid 3-band config", func(t *testing.T) {
		autoTrue := true
		configs := []BandConfig{
			{ThresholdDB: -30, Ratio: 2.0, KneeDB: 6.0, AttackMs: 20, ReleaseMs: 200, AutoMakeup: &autoTrue},
			{ThresholdDB: -20, Ratio: 4.0, KneeDB: 3.0, AttackMs: 10, ReleaseMs: 100},
			{ThresholdDB: -15, Ratio: 6.0, KneeDB: 0.0, AttackMs: 5, ReleaseMs: 50},
		}
