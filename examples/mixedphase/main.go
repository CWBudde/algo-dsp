// Command mixedphase compares two fixed-length mixed-phase FIR designs.
package main
...
func main() {
	...
	for _, delay := range []int{0, 8, 16, 32, 64} {
		...
		printMetrics("budde-iterative", delay, iterative.Metrics)
		...
		printMetrics("phase-interpolation", delay, direct.Metrics)
	}
}

func printMetrics(method string, delay int, metrics mixedphase.Metrics) {
	fmt.Printf(
		"%s,%d,%.9g,%.6f,%d,%.6f,%.9g\n",
		method,
		delay,
