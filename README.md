...
| `dsp/effects`          | Algorithmic audio effects: soft-knee compressor (log₂-domain, zero-alloc), time-domain pitch shifter (WSOLA), frequency-domain pitch shifter (phase vocoder), YIN pitch detection and auto-tune style pitch correction, channel vocoder (1/3-octave and Bark filter banks, optional multirate analysis), chorus (modulated delay), feedback delay, reverb (Schroeder/Freeverb). Optional fast-math mode (`-tags=fastmath`) for 26% speedup |
