...
| `dsp/effects`          | Algorithmic audio effects: soft-knee compressor (log₂-domain, zero-alloc), time-domain pitch shifter (WSOLA), frequency-domain pitch shifter (phase vocoder), chorus (modulated delay), reverb (Schroeder/Freeverb). Optional fast-math mode (`-tags=fastmath`) for 26% speedup |
