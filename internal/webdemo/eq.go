...
func (e *Engine) updateEQBand(dst **biquad.Chain, family, kind string, order int, freq, gainDB, q float64) {
	...
	// Same section count: overwrite coefficients in place so the existing
	// delay-line state is preserved; otherwise replace the chain wholesale.
	if (*dst).NumSections() == fresh.NumSections() {
		for i := range fresh.NumSections() {
			(*dst).Section(i).Coefficients = fresh.Section(i).Coefficients
		}

		(*dst).SetGain(fresh.Gain())
