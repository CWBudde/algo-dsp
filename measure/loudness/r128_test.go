...
func TestLoudness_Sine(t *testing.T) {
	fs := 48000.0
	f0 := 1000.0
	m := NewMeter(WithSampleRate(fs), WithChannels(1))
...
func TestLoudness_StereoSine(t *testing.T) {
	fs := 48000.0
	f0 := 1000.0
	m := NewMeter(WithSampleRate(fs), WithChannels(2))

	sig := testutil.DeterministicSine(f0, fs, 1.0, int(fs*4)) // 4 seconds
	m.StartIntegration()
	for _, s := range sig {
		m.ProcessSample([]float64{s, s}) // Coherent sine in both channels
	}
...
func TestLoudness_Gating(t *testing.T) {
	...
	highSig := testutil.DeterministicSine(1000, fs, 1.0, int(fs*10))
	lowSig := testutil.DeterministicSine(1000, fs, 0.0001, int(fs*10)) // -80 dB
	
	m.StartIntegration()
	for _, s := range highSig {
		m.ProcessSample([]float64{s})
	}
	highLoudness := m.Integrated()
	
	for _, s := range lowSig {
		m.ProcessSample([]float64{s})
	}
	totalLoudness := m.Integrated()
